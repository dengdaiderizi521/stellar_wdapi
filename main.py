...
class dyttplugin(StellarPlayer.IStellarPlayerPlugin):
    ...
    def _bgThread(self):
        while len(self.categories) == 0 and not self.isExit:
            self.parsePage()
            time.sleep(0.001)
        print(f'myapi bg thread:{self.gbthread.native_id} exit')
        # 刷新界面
        def update():
    ...
    def stop(self):
        if self.gbthread.is_alive():
            print(f'myapi bg thread:{self.gbthread.native_id} is still running')
        return super().stop()
    ...
    def onModalCreated(self, pageId):
        print(f'myapi onModalCreated {pageId=}')
        if pageId == 'main':
            if len(self.movies) == 0:
                self.loading()
        elif pageId != 'search':
